...
def send_email_over_smtps(server,frommail,tomail,subject,text,files=[]):
    ...
    for file in files:
        ...
        msg.attach(part)
    try:
        smtpserver = smtplib.SMTP(server['name'],port=server['port'],timeout=20) #此处需要填写服务器地址，587是默认smtps端口
    ...
    try:
        ...
        result=smtpserver.sendmail(frommail,tomail,msg.as_string())
        if result:
            for recipient in result:
                mesg="发送到 %s 的邮件被拒绝," % recipient + "错误码: %d,错误信息: %s" % result[recipient]
                print "\033[1;33m%s\033[0m" % mesg
    except Exception,e:
